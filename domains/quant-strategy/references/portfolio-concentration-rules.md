...
### 主题浓度分级约束（2026-07-01 修订）

- 单一子主题（如"AI HBM存储"：MU + DRAM）总仓位不得超过 NAV **25%**。
- 单一大主题（如"AI Capex"：GLW + TTMI + MXL + DRAM + MU 等）合计硬上限为 NAV **55%**。
- **0%-40%**：允许按市场风险门槛和个股技术信号正常分批加仓。
- **40%-50%**：进入集中管理区。只有当市场风险状态为 `normal`、宽度/信用没有明显恶化、没有待执行硬止损，且买入对象是本主题最强核心股时，才允许净加仓；单日该主题净增不得超过 NAV **5%**。
- **50%-55%**：只允许同主题强弱换仓，买入必须同时减掉等额弱仓，不得增加主题净敞口。
- **超过55%**：禁止买入并必须减仓回到硬上限内。
- 当市场处于 `normal`、主导主题趋势和相对强度明确，但暂无合格第二主题时，允许第一主题在上述条件内提高到 **50%**；不得仅因达到40%就长期强制账户保持过多现金。
- 总仓位与主题浓度分开管理。受控进攻模式在 `normal` 市场下仍以总股票仓位约 **45%-60%** 为近期运作区间，但所有加仓仍需通过个股趋势、延伸度、事件冷静期和止损检查。
- 规则以已确认真实持仓计算，不含挂单。

修订原因：原“达到40%后只能等额换仓”规则来自 2026-06-25 单日同主题过快建仓的事后教训，并无独立回测支持。分级规则继续限制追涨和相关性，同时减少强势正常市场中的结构性低仓位风险。

### 同日同主题新增上限

- 同一大主题下，单一交易日内新增仓位（按买入市值计）合计不得超过 NAV **15%**。
- 本规则叠加在单日总加仓速度规则之上，取更严格的一方约束。
- 实际案例（2026-06-25 教训）：DRAM + MXL + MU 同日买入使 AI Capex 主题从 22% 跳至 47%（新增25%），违反本规则。
