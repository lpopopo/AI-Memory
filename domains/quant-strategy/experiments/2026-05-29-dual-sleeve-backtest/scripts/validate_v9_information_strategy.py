...
def stats(curve,start="2026-06-01",end="2026-07-02"):
...
def main():
 ...
 for cost in (.001,.002,.005):
  result=V9Backtester(panels,vix,events,V9Config(transaction_cost=cost,source_healthy=raw["source_health"]=="healthy",source_failure_date=failed_on),updates).run();costs[str(cost)]=stats(result.equity);runs[str(cost)]=result
 # A healthy-source counterfactual is diagnostic only; the actual current source status is partial.
 research=V9Backtester(panels,vix,events,V9Config(transaction_cost=.001,source_healthy=True),updates).run();close=panels["close"]
 ...
 audit=[x for x in research.audit if x["date"]>="2026-06-01"]
 stock_days=sum(bool(x["stock_targets"]) for x in audit);max_single=max([max(x["stock_targets"].values(),default=0) for x in audit],default=0)
 reliable=split["counts"]["reliable"]
 ...
 out={"status":"research_only_not_promoted","data":meta,"event_store":{"count":len(events),"reliable_count":reliable,"source_health":raw["source_health"],"source_failure_date":failed_on,"split":split},"period":["2026-06-01","2026-07-02"],"comparisons":comparisons,"cost_sensitivity_actual_source_health":costs,"research_diagnostics":{"stock_invested_days":stock_days,"max_single_weight":max_single,"latest_audit":research.audit[-1]},"promotion_checks":gates,"promoted":all(gates.values()),"limitations":[f"Only {reliable} reliable timestamped events exist; no parameter optimization is permitted.","The period is too short for stable annualized statistics.","Source degradation blocks only new entries from its recorded failure date; earlier archived signals remain replayable."]}
