...
def growth_weights(config, close, indicators, dt, target_total, bull):
    ...
    if not selected:
        if config.fallback == "cash":
            return {}
        elif config.fallback == "spy":
            return {"SPY": target_total}
        else:
...
def candidate_configs():
    idx = 1
    # Focused parameter search to find optimal fallback strategy and sleeve balance
    for bull_value, normal_value, bear_growth, top_n, bull_rule, bear_rule, value_mode, fallback in itertools.product(
        [0.20, 0.30],                         # bull value weight (growth is 1 - value)
        [0.40, 0.50],                         # normal value weight (growth is 1 - value)
        [0.10, 0.20, 0.30],                   # bear growth weight (value is 1 - growth)
        [2, 3],                               # growth top N
        ["qqq100_m63", "qqq200_m126"],        # bull rule
        ["both_below_200", "both_below_200_negative_m63"], # bear rule
        ["top2"],                             # value mode
        ["cash", "spy", "qqq"],               # fallback parameter!
    ):
        yield V3Config(
            name=f"dual_sleeve_v3_refined_{idx:03d}",
...
def score_candidate(splits):
    ...
    return (
        ...
        + train["cagr"] * 0.6
        + test["cagr"] * 0.6
        + min(0.0, bear["cagr"] + 0.10) * 1.5
        + bear["max_drawdown"] * 0.3
    )


def write_v3_refined_summary(results, best, benchmarks):
    cfg = best["config"]
    lines = [
        ...
        "",
        f"- Name: `{cfg.name}`",
        f"- Fallback strategy: `{cfg.fallback}`",
    ...
    comparison = {"Dual Sleeve V3.1 Best": best["curve"], **benchmarks}
    ...
    for rank, item in enumerate(results[:10], start=1):
        ...
        cfg_text = (
            ...
            f"{cfg.bull_rule}, top{cfg.growth_top_n}"
        )
        lines.append(
            f"| {rank} | {cfg.name} | {cfg.fallback} | {pct(full['cagr'])} | {pct(full['max_drawdown'])} | {full['sharpe']:.2f} | "
    ...
    (RESULTS_DIR / "v3_refined_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main():
    ...
    print(f"Running optimization grid over {len(configs)} configurations...")
    ...
    best = results[0]
    best["curve"].to_csv(RESULTS_DIR / "v3_refined_best_equity_curve.csv", index_label="date")
    best["allocations"].to_csv(RESULTS_DIR / "v3_refined_best_monthly_allocations.csv", index=False)
    write_v3_refined_summary(results, best, benchmarks)
    ...
    print(f"Fallback Strategy: {best['config'].fallback}")
    print(f"Full CAGR: {pct(full['cagr'])}, Full MDD: {pct(full['max_drawdown'])}, Sharpe: {full['sharpe']:.2f}")
    print(f"Train CAGR: {pct(train['cagr'])}, Test CAGR: {pct(test['cagr'])}, Bear 2022 CAGR: {pct(bear['cagr'])}")
