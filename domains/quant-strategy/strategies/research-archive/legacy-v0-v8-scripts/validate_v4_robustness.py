...
from test_v4_stock_alpha import V4Config, fetch_all_data, prepare_indicators, run_v4
...
def write_summary(split_results, rolling_results):
    ...
    for key, value in asdict(BEST_V4).items():
    ...
    (RESULTS_DIR / "v4_robustness_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main():
    close = fetch_all_data()
    indicators = prepare_indicators(close)
    v4_curve, _allocations = run_v4(close, indicators, BEST_V4)
    v4_curve = v4_curve.rename("Dual Sleeve V4 Best")

    curves = {
        "Dual Sleeve V4 Best": v4_curve,
