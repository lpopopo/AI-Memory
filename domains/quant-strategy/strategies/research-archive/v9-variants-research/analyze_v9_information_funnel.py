 ...
 for event in events:
  ...
  for symbol in event.symbols:
   ...
   for dt in close.index[loc:min(loc+21,len(close.index))]:
    ...
    ok,path,technical,reason,high_vol=engine._tech_setup(symbol,dt,event.effective_at.normalize())
    score=event.source_completeness+event.thesis_novelty+engine._fundamental_score(event,symbol,dt)+technical-event.crowding_penalty
