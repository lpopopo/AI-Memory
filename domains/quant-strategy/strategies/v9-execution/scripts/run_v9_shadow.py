...
def initialize_mode_accounts(mode_dir: Path, mode: str, manifest: dict, config_base: dict) -> str:
    """Create genesis initial_state.json files for dry-run or forward rehearsal."""
    accounts_dir = mode_dir / "accounts"
    accounts_dir.mkdir(parents=True, exist_ok=True)
    frozen_at_utc = manifest.get("frozen_at_utc")
    code_hash = manifest.get("combined_code_hash", "")
    baseline_events_hash = manifest.get("baseline_event_snapshot_hash", "")
    frozen_base_config_hash = digest(config_base)

    created = []
    for acc_name, overrides in ACCOUNTS.items():
        acc_config = {**config_base, **overrides}
        ...
        base_state["state_hash"] = state_digest(base_state)
        acc_dir = accounts_dir / acc_name
        acc_dir.mkdir(parents=True, exist_ok=True)
        path = acc_dir / "initial_state.json"
        if path.exists():
            current = json.loads(path.read_text(encoding="utf-8"))
            if state_digest(current) != state_digest(base_state):
                raise TamperAlarmException(f"Tamper Alarm: {acc_name} initial_state.json differs from expected genesis!")
        else:
            atomic_freeze(path, base_state)
            created.append(acc_name)

    shared_dir = mode_dir / "shared"
    ...
    atomic_freeze(shared_dir / "event_chain_genesis.json", event_genesis)
...
def main():
    ...
    parser.add_argument("--as-of", required=False, help="Required unless --initialize only")
    ...
    if args.initialize:
        status = initialize_mode_accounts(mode_dir, mode, manifest, config_base)
        print(json.dumps({"initialize": status, "mode": mode, "mode_dir": str(mode_dir)}, indent=2))
        if args.as_of is None:
            return

    if args.as_of is None:
        raise SystemExit("--as-of is required unless running --initialize alone")
